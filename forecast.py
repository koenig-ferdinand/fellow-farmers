...
def get_daily_forecast(start_date, end_date=None):
    ...
    If end_date is None, uses start_date for both.
    ...
    sankt_gallen_coords = {"latitude": 47.4239, "longitude": 9.3767}
    measure_labels = (
        "TempAir_DailyAvg (C);"
        "TempAir_DailyMax (C);"
        "TempAir_DailyMin (C);"
...
def get_historical_weather(lat, lon, start_date, end_date, years=5):
    """
    Simulates retrieving historical weather data for the given location by averaging the same period over the past 'years' years.
    In a real implementation, replace this simulation with an API call or database query.
    """
    date_format = "%Y-%m-%d"
